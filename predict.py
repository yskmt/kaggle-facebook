...
test_info = pd.concat([test_info, test_info_append], axis=1)
...
# humans, match the numbers for each class
index_shuffle = range(num_bots)
np.random.shuffle(index_shuffle)
X_human = X_train[index_shuffle]
y = np.concatenate([y[:num_bots], y[num_human:]], axis=0)
...
# clf = SGDClassifier(loss="log", verbose=1, random_state=1234, n_iter=500)
clf.fit(X_train, y)
